...
if df is not None:
    ...
    with tab1:
        ...
        cols1[0].metric("Overall Score", f"{avg_scores['Overall Score']:.2f}", help=metric_tooltips["Overall Score"])
        cols1[1].metric("Patient Safety", f"{avg_scores['Patient Safety']:.2f}", help=metric_tooltips["Patient Safety"])
        cols1[2].metric("SOAP Compliance", f"{avg_scores['SOAP Compliance']:.1%}", help=metric_tooltips["SOAP Compliance"])
        cols1[3].metric("Clinical Accuracy", f"{avg_scores['Clinical Accuracy']:.2f}", help=metric_tooltips["Clinical Accuracy"])
        cols2[0].metric("Terminology Accuracy", f"{avg_scores['Terminology Accuracy']:.2f}", help=metric_tooltips["Terminology Accuracy"])
        cols2[1].metric("Hallucination Score", f"{avg_scores['Hallucination']:.2f}", help=metric_tooltips["Hallucination Score"])
        cols2[2].metric("Missing Info Score", f"{avg_scores['Missing Info']:.2f}", help=metric_tooltips["Missing Info Score"])

        st.header("Metric Scores Overview")
        # Create a bar chart for average scores
        metric_names = list(avg_scores.keys())[1:] # Exclude overall score from chart for better scale
        metric_values = [list(avg_scores.values())[i] for i in range(1, len(avg_scores))]

        fig = go.Figure(data=[go.Bar(x=metric_names, y=metric_values, text=[f'{v:.2f}' for v in metric_values], textposition='auto')])
        fig.update_layout(title_text='Average Scores by Metric', xaxis_title="Metric", yaxis_title="Average Score", yaxis=dict(range=[0,1]))
    ...
    with tab2:
        ...
        selected_index = st.selectbox("Select a note to review:", df.index, key='note_selector')
        ...
        if selected_index is not None:
            ...
            col1.metric("Safety", f"{note_data['clinical_safety_score']:.2f}", help=metric_tooltips["Patient Safety"])
            col2.metric("SOAP", f"{note_data['soap_structure_score']:.2f}", help=metric_tooltips["SOAP Compliance"])
            col3.metric("Accuracy", f"{note_data['clinical_accuracy_score']:.2f}", help=metric_tooltips["Clinical Accuracy"])
            col4.metric("Terminology", f"{note_data['medical_terminology_score']:.2f}", help=metric_tooltips["Terminology Accuracy"])
            col5.metric("Hallucination", f"{note_data['hallucination_score']:.2f}", help=metric_tooltips["Hallucination Score"])
            col6.metric("Missing Info", f"{note_data['missing_info_score']:.2f}", help=metric_tooltips["Missing Info Score"])
    ...
    st.warning("Evaluation results not found. Please run the evaluation first using `just run`.")
